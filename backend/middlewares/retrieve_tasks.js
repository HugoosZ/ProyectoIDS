...
// ... (Mantén tus funciones getAllTasks y getUserTasks de tu compañero, si las tiene) ...
...
exports.getUserTaskStatus = async (req, res) => {
    try {
        ...
        // --- DEBUG LOGS INICIO ---
        console.log("********** DEBUG retrieve_tasks.getUserTaskStatus - Inicio Petición **********");
        console.log("UID solicitado en URL (`:userId`):", userId);
        console.log("UID del usuario que solicita (desde token - `req.user.uid`):", requestingUserId);
        console.log("EmpresaId del usuario que solicita (desde token - `req.user.empresaId`):", requestingUserEmpresaId);
        ...
        if (!requestedUserDoc.exists) {
            console.log("DEBUG: Usuario solicitado NO encontrado en Firestore:", userId);
        ...
        console.log("DEBUG: EmpresaId del usuario solicitado (desde Firestore):", requestedUserEmpresaId);
        ...
        if (!requestingUserIsAdmin) {
            if (userId !== requestingUserId) {
                console.log("DEBUG: Acceso denegado - Usuario NO admin intentó ver tareas de OTRO usuario.");
                return res.status(403).json({ error: "No autorizado: Un usuario regular solo puede ver sus propias tareas." });
            }
            ...
            if (requestingUserEmpresaId !== requestedUserEmpresaId) {
                console.log("DEBUG: Acceso denegado - Usuario intentó ver sus propias tareas pero la empresaId no coincide (anomalía).");
                return res.status(403).json({ error: "No autorizado: Discrepancia en empresa." });
            }
        } else {
            // Si es un admin, puede ver tareas de cualquier usuario de SU MISMA EMPRESA.
            if (requestingUserEmpresaId !== requestedUserEmpresaId) {
                console.log(`DEBUG: Acceso denegado - Admin de empresa [<span class="math-inline">\{requestingUserEmpresaId\}\] intentó ver tareas de usuario de OTRA empresa \[</span>{requestedUserEmpresaId}].`);
                return res.status(403).json({ error: "Acceso denegado: El administrador no puede ver tareas de otras empresas." });
            }
            console.log(`DEBUG: Admin de empresa [<span class="math-inline">\{requestingUserEmpresaId\}\] tiene permiso para ver tareas del usuario \[</span>{userId}] de su misma empresa.`);
        }
        ...
        if (today === "true") {
            ...
            console.log(`DEBUG: Filtro por tareas de HOY (startTime entre ${startDate.toDate()} y ${endDate.toDate()}).`);
        } else if (week === "true") {
            const { startDate, endDate } = getDateRange("week");
            tasksQuery = tasksQuery
                .where("startTime", ">=", startDate)
                .where("startTime", "<", endDate); // Usar < para semana completa
            console.log(`DEBUG: Filtro por tareas de la SEMANA (startTime entre ${startDate.toDate()} y ${endDate.toDate()}).`);
        }
        ...
        console.log(`DEBUG: <span class="math-inline">\{tasks\.length\} tareas encontradas para el usuario \[</span>{userId}].`);
        console.log("********** DEBUG retrieve_tasks.getUserTaskStatus - Fin Petición Exitosa **********");
        ...
        console.error("********** DEBUG retrieve_tasks.getUserTaskStatus - Error en la petición **********");
        console.error("Error al obtener tareas (getUserTaskStatus):", error);

        if (error.code === 'failed-precondition') {
            console.error("DEBUG: Posible error de índice en Firestore. Verifique la consola de Firebase. URL del índice sugerido por Firebase.");
            return res.status(400).json({
                error: "Error en la consulta de base de datos. Asegúrate de tener los índices necesarios en Firestore.",
                details: error.message
            });
        }
        if (error.code === 'PERMISSION_DENIED') {
            console.error("DEBUG: Error de PERMISOS de Firestore. Revise sus reglas de seguridad. Puede que sus reglas estén bloqueando esta consulta.");
            return res.status(403).json({
                error: "Error de permisos en la base de datos.",
                details: error.message
            });
        }
        if (error.code === 3) { // Este es un error común de Firestore para consultas inválidas
            return res.status(400).json({ error: "Parámetros de consulta inválidos.", details: error.message });
        }
...
// ... (Asegúrate de que las otras funciones como getAllTasks y getUserTasks estén también si tu compañero las incluyó) ...
