...
exports.createUser = async (req, res) => {
    ...
    try {
        ...
        if (req.user && req.user.isAdmin && req.user.empresaId) {
            ...
            console.log("DEBUG: Heredando empresaId del admin autenticado:", finalEmpresaId);
        } else if (isAdmin) { // Si el usuario que se está CREANDO es un admin, y no se hereda un ID (porque no hay admin padre o no tiene ID)
            ...
            console.log("DEBUG: Generando nuevo empresaId para el primer admin:", finalEmpresaId);
        } else {
            ...
            console.warn("ADVERTENCIA: Creando usuario sin empresaId heredado ni especificado. Generando uno nuevo.");
        ...
        const newUser = await authService.createUserWithRole({
            email, password, rut, name, lastName, role, isAdmin, empresaId: finalEmpresaId
        });
        ...
        } else if (error.message.includes('contraseña debe tener al menos 6 caracteres')) {
